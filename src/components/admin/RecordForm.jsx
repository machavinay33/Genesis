...
export default function RecordForm({ columns, initialValues, onSubmit, onCancel, submitLabel = 'Save' }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="border border-ink/10 bg-white p-5 shadow-sm">
      <div className="grid gap-4 sm:grid-cols-2">
        {columns.map((col) => (
          <div key={col.key} className={col.type === 'textarea' || col.type === 'photo' ? 'sm:col-span-2' : ''}>
            ...
            {col.type === 'textarea' && (
              <textarea
                className="w-full border border-ink/15 px-3 py-2 text-sm focus:border-genesis focus:outline-none"
                rows={3}
      ...
      <div className="mt-5 flex gap-3">
        <button
          ...
          className="bg-ink px-5 py-2 text-xs font-semibold uppercase tracking-wide text-white transition-colors hover:bg-genesis disabled:opacity-50"
        >
        ...
        <button
          type="button"
          onClick={onCancel}
          className="border border-ink/20 px-5 py-2 text-xs font-semibold uppercase tracking-wide text-ink/70 hover:border-ink/40"
        >
          Cancel
        </button>
      </div>
